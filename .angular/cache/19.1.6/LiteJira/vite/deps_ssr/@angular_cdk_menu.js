...
var MenuStack = class _MenuStack {
  ...
  _empty = new import_rxjs.Subject();
  /** Emits whether any menu in the menu stack has focus. */
  _hasFocus = new import_rxjs.Subject();
  /** Observable which emits the MenuStackItem which has been requested to close. */
  closed = this._close;
  /** Observable which emits whether any menu in the menu stack has focus. */
  hasFocus = this._hasFocus.pipe((0, import_operators.startWith)(false), (0, import_operators.debounceTime)(0), (0, import_operators.distinctUntilChanged)());
  /**
...
var CdkMenuTriggerBase = class _CdkMenuTriggerBase {
  ...
  stopOutsideClicksListener = (0, import_rxjs.merge)(this.closed, this.destroyed);
...
var CdkMenuTrigger = class _CdkMenuTrigger extends CdkMenuTriggerBase {
  ...
  _registerCloseHandler() {
    if (!this._parentMenu) {
      this.menuStack.closed.pipe((0, import_operators.takeUntil)(this.destroyed)).subscribe(({
        item
      }) => {
  ...
  _subscribeToOutsideClicks() {
    if (this.overlayRef) {
      this.overlayRef.outsidePointerEvents().pipe((0, import_operators.takeUntil)(this.stopOutsideClicksListener)).subscribe((event) => {
  ...
  _subscribeToMenuStackHasFocus() {
    if (!this._parentMenu) {
      this.menuStack.hasFocus.pipe((0, import_operators.takeUntil)(this.destroyed)).subscribe((hasFocus) => {
        if (!hasFocus) {
          this.menuStack.closeAll();
        }
      });
...
var PointerFocusTracker = class {
  ...
  entered = new import_rxjs.Subject();
  /** Emits when an element is moused out. */
  exited = new import_rxjs.Subject();
  ...
  _bindEvents() {
    this._itemsSubscription = this._items.changes.pipe((0, import_operators.startWith)(this._items)).subscribe(() => {
      this._cleanupEvents();
      this._eventCleanups = [];
...
var CdkMenuBase = class _CdkMenuBase extends CdkMenuGroup {
  ...
  _subscribeToMenuOpen() {
    const exitCondition = (0, import_rxjs.merge)(this.items.changes, this.destroyed);
    this.items.changes.pipe((0, import_operators.startWith)(this.items), (0, import_operators.mergeMap)((list) => list.filter((item) => item.hasMenu).map((item) => item.getMenuTrigger().opened.pipe((0, import_operators.mapTo)(item), (0, import_operators.takeUntil)(exitCondition)))), (0, import_operators.mergeAll)(), (0, import_operators.switchMap)((item) => {
      this.triggerItem = item;
      return item.getMenuTrigger().closed;
    }), (0, import_operators.takeUntil)(this.destroyed)).subscribe(() => this.triggerItem = void 0);
  }
  ...
  _handleFocus() {
    this._focusMonitor.monitor(this.nativeElement, false).pipe((0, import_operators.takeUntil)(this.destroyed)).subscribe((origin) => {
...
var CdkMenu = class _CdkMenu extends CdkMenuBase {
  ...
  _subscribeToMenuStackEmptied() {
    this.menuStack.emptied.pipe((0, import_operators.takeUntil)(this.destroyed)).subscribe((event) => this._toggleMenuFocus(event));
  }
...
var CdkMenuBar = class _CdkMenuBar extends CdkMenuBase {
  ...
  /** Subscribe to the MenuStack emptied events. */
  _subscribeToMenuStackEmptied() {
    this.menuStack?.emptied.pipe((0, import_operators.takeUntil)(this.destroyed)).subscribe((event) => this._toggleOpenMenu(event));
  }
  static ɵfac = /* @__PURE__ */ (() => {
...
var CdkContextMenuTrigger = class _CdkContextMenuTrigger extends CdkMenuTriggerBase {
  ...
  _subscribeToOutsideClicks(userEvent) {
    if (this.overlayRef) {
      ...
      if (userEvent) {
        const [auxClicks, nonAuxClicks] = (0, import_rxjs.partition)(outsideClicks, ({
          type
        }) => type === "auxclick");
        outsideClicks = (0, import_rxjs.merge)(
          ...
          nonAuxClicks.pipe((0, import_operators.skipWhile)((event, index) => userEvent.ctrlKey && index === 0 && event.ctrlKey)),
          // If the menu was triggered by the `contextmenu` event, skip the first `auxclick` event
          // because it fires when the mouse is released on the same click that opened the menu.
          auxClicks.pipe((0, import_operators.skip)(1))
        );
      }
      outsideClicks.pipe((0, import_operators.takeUntil)(this.stopOutsideClicksListener)).subscribe((event) => {
